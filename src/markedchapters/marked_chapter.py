...
class MarkedChapterParser:
    ...
    def on_list_item(self, idx, white_spaces, item_marker, item_data):
        ...
        if isinstance(item_data, ChLink):
            ...
            except MarkedChaptersException as e:
                raise MarkedChaptersException(
                    'Error during adding url to chapter toc\n' +
                    f'Source:\n' +
                    get_source_snippet_from_text(self.__text, idx + 1)
                    ,
                    e  # TODO: this was synamicErrorS previously, refactor parameters accordingly
                )
...
class ChaptersTOC:
    ...
    def add_chapter(self, ch_link: ChLink):
        ...
        if href.startswith('/'):
            raise MarkedChaptersException(f'Chapter link must be relative: {href}')
        marked_extensions = self.site.system_settings['configs.marked_extensions'].as_tuple
        if not href.endswith(marked_extensions):
            raise MarkedChaptersException(f'Chapter link must be a relative path to a file with an extension of marked content'
                               f' {marked_extensions}: {href}')
        if href.endswith('/'):
            raise MarkedChaptersException(f'Chapter link must be a relative file name to a marked content: {href}')
